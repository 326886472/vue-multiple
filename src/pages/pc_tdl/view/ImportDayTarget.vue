    ...
    <div v-loading="loading" class="importBox">
      ...
      <div class="title">
        <div class="title-text">
          <span class="title-text-span">导入日计划</span>
        </div>
      ...
      <div v-show="list.length" id="importTargetList" ref="list" v-infinite-scroll="load" class="list" :infinite-scroll-disabled="scroll.disabled" :infinite-scroll-immediate="false">
        <div v-for="(item, index) in list" :key="index" class="item">
          <div class="top">
            ...
            <div class="top_title">
              日计划{{ moment(item.day).format('YYYY/MM/DD') }}
            </div>
          ...
          <div class="content">
            ...
            <div v-if="!item.loadAll" class="item_probably">
              计划1: {{ item.target_list[0]?.content }}
            </div>
            <div v-else>
              <!-- 总结 -->
                ...
                <div v-for="(target, index2) in item.target_list" :key="index2" class="target_item">
                  <div class="item_list_top">
                    ...
                    <div class="time">
                      时间
                      <span>
                        {{ moment(item.day).format('MM月DD日') }}{{ target.from }}-{{ target.to }}
                      </span>
                    </div>
                    <div class="type">
                      分类
...
export default {
  ...
  methods: {
    ...
    async getList() {
      this.loading = true
      const res = await api.tdlDayImportList({
